...
if (isset($_GET['invoice'])) {
  ...
  $invoice = htmlentities(@mysqli_real_escape_string($con, $_GET['invoice']));
  ...
  if (mysqli_num_rows($selectInvoice) > 0) {
    ...
    if (@$_SESSION['user_role'] == 1) {
      ...
      if ($fetchInvoice['user_id'] != $_SESSION['user_id']) {
...
?>
    ...
    <body>
      ...
      <center id="account"> <button onclick="printdiv('invoice');" id="print">Print</button> &nbsp;&nbsp; <?php if (isset($_SESSION['user_id'])) { ?>
          <!-- <a href="account" id="accountbtn">My Account</a> -->
      </center>
    <?php } else { ?>
      <a href="<?= $url ?>" id="accountbtn">Website</a></center>
    <?php } ?>
          ...
          <div class="equalHWrap eqWrap top">
            <div class="equalHW eq  logo-block">
              ...
              <?php if ($fetchInvoice['user_id'] != 140) { ?>
                ...
                Address:<?= $fetchInvoice['address'] ?> <br><?= $fetchInvoice['city'] ?><br> <?= $fetchInvoice['zip'] ?>
              <?php } else { ?>
            ...
            <div class="equalHW eq title-block  pull-right">
              ...
              <div style="border:1px solid; padding: 10px">
                ...
                <b>Order Date <?= $date = date('Y-m-d', strtotime($fetchInvoice['created'])); ?></b><br>
                <b>Tracking Id <?= $fetchInvoice['track_id']; ?></b>
              </div>
            </div>
          </div>
          <?php
...
<?php }elseif (mysqli_num_rows($selectInvoice) < 1) {
    $selectInvoices = mysqli_query($con, "SELECT users.email,users.city,users.zip,users.address,users.firstname,users.lastname,invoice.* FROM `invoice` join users on users.id=invoice.user_id WHERE invoice.invoice_number='$invoice'");
    $fetchInvoice = mysqli_fetch_array($selectInvoices);
    if (@$_SESSION['user_role'] == 1) {
    } else {
      if ($fetchInvoice['user_id'] != $_SESSION['user_id']) {
        // echo"<script>window.open('index','_self');</script>";
