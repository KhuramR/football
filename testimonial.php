...
<head>
    ...
    <title>Testimonial - <?= $fetchWebsite['site_name'] ?></title>
...
<body data-spy="scroll" data-target="#navSection" data-offset="100">
    ...
    <div class="main-container" id="container">
        ...
        <div id="content" class="container">
            ...
            $selectProduct = mysqli_query($con, "SELECT * FROM testimonials where id='$id'");
                            ...
                            <form id="<?=isset($_GET['edit'])?"update" :"add"?>">
                                <div class="form-row mb-4">
                                    <div class="form-group col-md-6">
                                        <label for="inputEmail4">Testimonial Title</label>
                                        <input type="text" name="title" class="form-control" id="inputEmail4" value="<?= @$fetchProduct['title'] ?>" <?=isset($_GET['edit'])?"" :"required=''"?> placeholder="Product Name">
                                        <input type="hidden" value="back" id="design">
                                        <input type="hidden" id="page" value="testimonials">
                                        <input type="hidden" name="id" value="<?=@$id?>">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label for="inputPassword4">Testimonial Image (100x100)</label>
                                        <input type="file" name="image" class="form-control"  accept="image/*" <?=isset($_GET['edit'])?"" :"required=''"?>  id="inputPassword4">
                                    </div>
                                </div>
                                <div class="form-group mb-4">
                                    <label for="inputAddress">Testimonial Short Description</label>
                                    <textarea class="form-control" name="short_description" placeholder="Testimonial Short Description" <?=isset($_GET['edit'])?"" :"required=''"?>><?= @base64_decode($fetchProduct['description']) ?></textarea>
                                </div>
                                <button type="submit" class="btn btn-primary mt-3 "><?=isset($_GET['edit'])?"Edit" :"Add"?> Testimonial</button>
                            </form>
                        </div>
                    </div>
                </div>
