                                ...
                                <div class="table-responsive mb-4">
                                    ...
                                    <table id="style-3" class="table style-3  table-hover">
                                        ...
                                        <tbody>
                                            <?php
                                            $i = 0;
                                            $selectProduct = mysqli_query($con, "SELECT * FROM `quiz_questions`  order by id desc");
                                            ...
                                            ?>
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        <?php 
                                                        if($fetchProduct['video'] != ''){

                                                        ?>
                                                        <span>
                                                            
                                                            <a data-fancybox="gallery" href="../uploads/quiz/<?= $fetchProduct['video'] ?>" class="fancy">
                                                            <video src="../uploads/quiz/<?= $fetchProduct['video'] ?>" class="profile-img" style="width: 50px;height: 50px;"></video>
                                        <i class="fa fa-play icoe blink" aria-hidden="true"></i>
                                    </a>
                                                        </span>
                                                   <?php }else{
                                                       echo'-';
                                                    ...
                                                    <td><?= $fetchProduct['question'] ?></td>
                                                    <td><?php
                                                    $qid = $fetchProduct['id'];
                                                    $select_option = mysqli_query($con,"SELECT * FROM `options` JOIN quiz_questions on options.question_id = quiz_questions.id WHERE options.question_id = '$qid' ");
                                                    while ($fetch_option =  mysqli_fetch_array($select_option)) {
                                                        if($fetch_option['options']!=''){
                                                        echo $fetch_option['options'].', ';
                                                    }
                                                    }
                                                    ...
                                                    $select_option = mysqli_query($con,"SELECT * FROM `options` JOIN quiz_questions on options.question_id = quiz_questions.id WHERE options.question_id = '$qid' ");
                                                    while ($fetch_option =  mysqli_fetch_array($select_option)) {
                                                        if($fetch_option['answers']!=''){
                                                        echo $fetch_option['answers'].', ';
                                                    }
                                                    }
